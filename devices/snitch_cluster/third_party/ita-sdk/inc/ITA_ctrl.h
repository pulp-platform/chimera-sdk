                    ...
                    if (i == 0 && j == 0) {
                        ...
                        if (prev_output_tile_l2 != 0 && prev_output_tile_l1 != 0 &&
                            ...
                            snrt_dma_start_1d((void *)prev_output_tile_l2,
                                              (void *)prev_output_tile_l1, prev_output_tile_size);
                        }
                    } else {
            ...
            for (int k = 0; k < N_TILE_EMBEDDING_SPACE; k++) {
                ...
                if (is_last_tile) {
                    if (N_TILE_SEQUENCE_LENGTH == 1 && N_TILE_PROJECTION_SPACE == 1) {
                        ...
                        snrt_dma_start_1d((void *)next_step_input_b_tile_l1,
                                          (void *)prev_output_tile_l1, next_step_input_b_tile_size);
                        snrt_dma_wait_all();
                    } else {
                        snrt_dma_start_1d((void *)next_step_input_b_tile_l1,
                                          (void *)next_step_input_b_tile_l2,
    ...
    for (int j = 0; j < N_TILE_SEQUENCE_LENGTH; j++) {
        for (int k = 0; k < N_TILE_PROJECTION_SPACE; k++) {
            ...
            snrt_dma_start_1d((void *)input_Pq_l1[index_buff],
                              (void *)(input_Pq_row_l2 + offset_input_Pq), tile_size_input_Pq);
            ...
            if (is_last_tile) {
                ...
                snrt_dma_start_1d((void *)input_Pk_l1[!index_buff],
                                  (void *)(input_Pk_row_l2 + offset_input_Pk), tile_size_input_Pk);
            }
        ...
        for (int k = 0; k < N_TILE_SEQUENCE_LENGTH; k++) {
            ...
            if (N_TILE_SEQUENCE_LENGTH > 1) {
                ...
                snrt_dma_start_1d((void *)input_qk_l1[index_buff],
                                  (void *)(input_qk_row_l2 + offset_input_qk), tile_size_input_qk);
            } else {
            ...
            if (is_last_tile) {
                snrt_dma_start_1d((void *)next_step_input_b_tile_l1,
                                  (void *)next_step_input_b_tile_l2, next_step_input_b_tile_size);
            } else {
                ...
                snrt_dma_start_1d((void *)input_Pv_l1[!index_buff],
                                  (void *)(input_Pv_row_l2 + offset_input_Pv), tile_size_input_Pv);
            }
...
static void ita_compute_output_embedding_step(
    const int8_t *const input_av_l2, const int8_t *const input_Wo_l2,
    const ita_int24_t *const bias_l2, const int8_t *output_l2, const int8_t *input_av_l1[2],
    const int8_t *input_Wo_l1[2], const ita_int24_t *bias_l1, const int8_t *output_l1[2],
    const int8_t *const prev_output_tile_l2, const int8_t *prev_output_tile_l1,
    ...
    for (int i = 0; i < N_TILE_SEQUENCE_LENGTH; i++) {
        for (int j = 0; j < N_TILE_EMBEDDING_SPACE; j++) {
            ...
            snrt_dma_start_1d((void *)bias_l1, (void *)(bias_l2 + offset_bias),
                              TILE_SIZE_EMBEDDING_SPACE * sizeof(ita_int24_t));
            ...
            for (int k = 0; k < N_TILE_PROJECTION_SPACE; k++) {
                ...
                if (!is_last_tile) {
                    ...
                    snrt_dma_start_1d((void *)input_Wo_l1[!index_buff],
                                      (void *)(input_Wo_l2 + offset_input_Wo), tile_size_input_Wo);
                }
...
static inline void __attribute((always_inline))
ita_compute_matmul_L1(const int8_t *input_a_l1, const int8_t *input_b_l1,
                      const ita_int24_t *bias_l1, const int8_t *output_l1) {
    for (int i = 0; i < N_TILE_SEQUENCE_LENGTH; i++) {
        for (int j = 0; j < N_TILE_PROJECTION_SPACE; j++) {
            const int offset_bias = j * TILE_SIZE_PROJECTION_SPACE;
            ...
            for (int k = 0; k < N_TILE_EMBEDDING_SPACE; k++) {
                ...
                ita_set_flags((uint32_t)ITA_FLAGS(is_first_tile, !is_last_tile, 0,
                                                  0, k == N_TILE_EMBEDDING_SPACE - 1 ? 0 : 1));
...
ita_compute_matmul_L2(const int8_t *const input_a_l2, const int8_t *const input_b_l2,
  ...
  for (int i = 0; i < N_TILE_SEQUENCE_LENGTH; i++) {
    for (int j = 0; j < N_TILE_PROJECTION_SPACE; j++) {
      ...
      for (int k = 0; k < N_TILE_EMBEDDING_SPACE; k++) {
        const int is_first_tile = (i == 0) && (j == 0) && (k == 0);
        const int is_last_tile = (i == N_TILE_SEQUENCE_LENGTH - 1) && (j == N_TILE_PROJECTION_SPACE - 1) &&
                                 (k == N_TILE_EMBEDDING_SPACE - 1);

        // Fetch inputs
        const int tile_size_input_a = TILE_SIZE_SEQUENCE_LENGTH * TILE_SIZE_EMBEDDING_SPACE;
        const int offset_input_a = (i * N_TILE_EMBEDDING_SPACE + k) * tile_size_input_a;
        snrt_dma_start_1d((void *)input_a_l1[index_buff], (void *)(input_a_l2 + offset_input_a), tile_size_input_a);

        const int index_buff_output = (i * N_TILE_PROJECTION_SPACE + j) % 2;

        ita_set_addresses(
            (uint32_t)input_a_l1[index_buff], (uint32_t)input_b_l1[index_buff],
            (uint32_t)input_b_l1[!index_buff], (uint32_t)bias_l1, (uint32_t)output_l1[index_buff_output]);
        ita_set_flags((uint32_t)ITA_FLAGS(is_first_tile, !is_last_tile, 0, 0, k == N_TILE_EMBEDDING_SPACE - 1 ? 0 : 1));
        ...
        if (!is_last_tile) {
          ...
          snrt_dma_start_1d((void *)input_b_l1[!index_buff], (void *)(input_b_l2 + offset_input_b), tile_size_input_b);
        }
        ...
        if (k == 0) {
          if (i != 0 || j != 0) {
            ...
            snrt_dma_start_1d((void *)(output_l2 + offset_output), (void *)output_l1[index_buff_output],
                              tile_size_output);
          }
        }
