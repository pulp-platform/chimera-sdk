...
// The matrix shape is selected at configure time via MATMUL_L1_SHAPE;
// see this test's CMakeLists.txt.
#ifndef TESTINPUTS_HEADER
#define TESTINPUTS_HEADER "testinputs_32x48x64.h"
#endif
#ifndef TESTOUTPUTS_HEADER
#define TESTOUTPUTS_HEADER "testoutputs_32x48x64.h"
#endif
 ...
 */
KEEP int32_t fll_matmul_test(void *args) {
    ...
    if (snrt_is_compute_core()) {
        float ops =
            ((float)MAT_OPS * test_args->repetitions) / snrt_cluster_compute_core_num(); // MACs
