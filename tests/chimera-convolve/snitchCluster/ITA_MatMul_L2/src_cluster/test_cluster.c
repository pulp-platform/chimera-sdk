...
int32_t ita_matmul_l2_test(void *args) {
    ...
    ita_matmul_l2_test_args_t *user_args = (ita_matmul_l2_test_args_t *)(test_args->args);
    ...
    if (snrt_is_dm_core()) {
        ...
        // L2 buffers
        const int8_t *output_L2 = user_args->output_L2;
        ...
        for (int i = 0; i < test_args->repetitions; i++) {
            if (i == test_args->repetitions - 1) {
                ...
                snrt_dma_start_1d((void *)output_L2, (void *)snrt_zero_memory_ptr(),
            ...
            ita_compute_matmul_L2(input_q,       // input_a_l3
                                  input_Wq[0],   // input_b_l3
                                  input_Bq[0],   // bias_l3
                                  output_L2,     // output_l3
                                  input_a0_buff, // input_a_l1
                                  input_b0_buff, // input_b_l1
                                  input_bias_0,  // bias_l1
                                  output_0_buff  // output_l1
            );

            ita_wait_job();

            const int prev_i_output = N_TILE_SEQUENCE_LENGTH - 1;
            const int prev_j_output = N_TILE_PROJECTION_SPACE - 1;
            const int tile_size_output = TILE_SIZE_SEQUENCE_LENGTH * TILE_SIZE_PROJECTION_SPACE;
            const int offset_output =
                (prev_i_output * N_TILE_PROJECTION_SPACE + prev_j_output) * tile_size_output;
            const int8_t *interm_output_tile_ptr = output_L2 + offset_output;
            const int index_buff_output =
                (N_TILE_SEQUENCE_LENGTH * N_TILE_PROJECTION_SPACE - 1) % 2;

            snrt_dma_start_1d((void *)interm_output_tile_ptr,
                              (void *)output_0_buff[index_buff_output], tile_size_output);
            snrt_dma_wait_all();
        }
        ...
        tot_err = check(1, (uint8_t *)output_L2, (uint8_t *)golden_interm_Pq,
